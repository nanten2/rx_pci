...
from rx_pci_single_ros import num_msg
...
class save_logger_high(object):
    ...
    def callback_num(self, req):
        if self.flag == 1:
            return
        filename = home_dir + self.timestamp + '/num.txt'
        self.timestamp_sisbb = req.timestamp
        self.num = req.num
        msg = '{self.timestamp} {self.num}\n'.format(**locals())
        f = open(filename, 'a')
        f.write(msg)
        f.close()
        return
...
if __name__ == '__main__':
    ...
    sub_num = rospy.Subscriber('num', num_msg, st.callback_num, queue_size=1)
    rospy.spin()
